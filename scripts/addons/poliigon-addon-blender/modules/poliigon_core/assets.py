...
SIZES = ["256", "512"] + [f"{i+1}K" for i in range(18)] + ["HIRES", "WM"]
...
class Texture(BaseTex):
        ...
        if local_only:
            ...
            for workflow in self.maps:
                for tex_map in self.maps[workflow]:
                    if tex_map.map_type.get_convention() == convention_min:
    ...
    def get_maps(self,
                 ...
                 lod: Optional[str] = None,
                 prefer_16_bit: bool = False,
                 suffix_list: List[str] = [".png", ".tif", ".tiff", ".jpg"],
                 variant: Optional[str] = None,
                 effective: bool = True
                 ) -> List[TextureMap]:
        ...
        for tex_map in self.maps[workflow]:
            ...
            tex_map_dict[map_type] = tex_map_dict.get(map_type, []) + [tex_map]
...
class Hdri(BaseTex):
    ...
    def get_size_list(self,
        ...
        return self.light.get_size_list(
    ...
    def get_maps(self,
        ...
        tex_maps = self.bg.get_maps(
            workflow, size, lod, prefer_16_bit, variant=variant, effective=effective)
...
class AssetData:
    ...
    def update(self, asset_data_new, purge_maps: bool = False) -> None:
        ...
        self.local_convention = asset_data_new.local_convention
