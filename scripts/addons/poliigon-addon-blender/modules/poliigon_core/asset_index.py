...
class AssetIndex():
    ...
    def __init__(self,
        ...
        if log is None:
            self.logger = logger.get_addon_logger().initialize_logger(
    ...
    def _decode_tex_convention_1(
            self,
            asset_dict: Dict
    ) -> Tuple[Dict, List[str], Tuple[float, float]]:
        ...
        return maps_dictionary, sizes, dimensions
    ...
    def _construct_texture(self,
        ...
        if convention == 1:
            maps, sizes, dimension = self._decode_tex_convention_1(asset_dict)
            variants = []
        else:
    ...
    def _prepare_hdri_update_asset_data(texture_maps: Dict[str, assets.TextureMap],
        ...
        for workflow_tex, tex_map_list in texture_maps.items():
            ...
            for tex_map in tex_map_list:
                is_bg = tex_map.map_type.get_effective() == assets.MapType.ENV
                is_bg |= tex_map.map_type.get_effective() == assets.MapType.JPG
                is_light = tex_map.map_type.get_effective() == assets.MapType.LIGHT
                is_light |= tex_map.map_type.get_effective() == assets.MapType.HDR
    ...
    @staticmethod
    def _prepare_model_update_asset_data(meshes: List[assets.ModelMesh],
                                         texture_maps: Dict[str, assets.TextureMap],
                                         sizes: List[str],
                                         variants: List[str],
                                         lods: List[str],
        ...
        files_found = (len(texture_maps) + len(meshes)) > 0
        if not files_found:
            return False
    ...
    def update_all_local_assets(self,
        ...
        # TODO(Andreas): value for ignore_old_names
