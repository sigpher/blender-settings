...
class CyclesMaterial():
    ...
    def try_to_assign_non_color_space(self, image: bpy.types.Image) -> None:
        ...
        colorspace_settings = type(image).bl_rna.properties["colorspace_settings"]
    ...
    def configure_tex_node_image(
        ...
        if name_tex in bpy.data.images.keys():
            ...
            image = bpy.data.images.load(path_tex)
            image.name = name_tex
        ...
        colorspace, alpha, interpolation = MAP_TYPE_TO_CYCLES_IMG_PARAMS[
            map_type_effective]
        if colorspace == "Raw":
            self.try_to_assign_non_color_space(image)
        else:
    ...
    def connect_emission(self) -> None:
        ...
        if bpy.app.version >= (4, 0):
            node_bsdf.inputs[27].default_value = 0.0
            self.mat.node_tree.links.new(
                node_bsdf.inputs[26], node_tex_emission.outputs[0])
    ...
    def connect_fabric(self) -> None:
        ...
        if bpy.app.version >= (4, 0):
            ...
            node_bsdf.inputs[23].default_value = 1.0  # weight
            node_bsdf.inputs[24].default_value = 0.3  # roughness
        else:
    ...
    def connect_sss(self) -> None:
        ...
        if bpy.app.version >= (4, 0):
            ...
            node_bsdf.inputs[7].default_value = 1.0
            node_bsdf.inputs[9].default_value = 0.1  # Subsurface Scale
    ...
    def connect_transmission(self) -> None:
        ...
        if bpy.app.version >= (4, 0):
            self.mat.node_tree.links.new(
                node_bsdf.inputs[17], node_tex_transmission.outputs[0])
        else:
            self.mat.node_tree.links.new(
